...
balt_solved_logistic = 
  ...
  balt_solved_logistic %>% 
    ...
    mutate(odds_ratio = exp(estimate),
           adj_conf_low = exp(conf.low), 
           adj_conf_high = exp(conf.high),
           term = recode(term, victim_sexMale = "victim_sex: male", 
                         victim_race_whitenonwhite = "victim_race: nonwhite")) %>% 
    select(Term = term, "Adj Odds Ratio" = odds_ratio, "Adj Conf: Lower bound" = adj_conf_low, 
           "Adj Conf: Upper bound" = adj_conf_high) %>% 
...
homocide_odds_city = 
  ...
  mutate(log_models = map(.x = data, glm_homicide_function),
         tidy_models = map(log_models, broom::tidy, conf.int = TRUE))  %>% 
